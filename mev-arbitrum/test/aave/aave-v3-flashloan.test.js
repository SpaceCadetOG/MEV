...
describe("Aave", function () {
  ...
  async function deployFixture() {
    ...
    borrowAmount = 1573n * 10n ** 18n; // 10000 dai
    fee = 10n * 10n ** 18n; // 10000 dai

    fundAmount2 = 200n * 10n ** 6n; // 20000 dai
    borrowAmount2 = 1573n * 10n ** 6n; // 10000 dai
    ...
    console.log(
      "Before Flashloan: DAI Balance of Owner",
    ...
    console.log(
      ...
      ethers.utils.formatUnits(await dai.balanceOf(loan.address), 18)
    );
  ...
  describe("1) AaveLending Contract", () => {
    describe("Flashloan", () => {
      it("it should fl usdc", async function () {
        const { loan, owner, usdc, fundAmount, dai, borrowAmount2 } =
          await loadFixture(deployFixture);
        await loan.MakeMoney(usdc.address, borrowAmount2, owner.address);

        console.log(
          "After Flashloan: USDC Balance of owner",
          ethers.utils.formatUnits(await usdc.balanceOf(owner.address), 6)
        );

      });

      it("it should fl dai", async function () {
        const { loan, owner, usdc, fundAmount, dai, borrowAmount } =
          await loadFixture(deployFixture);
        await loan.MakeMoney(dai.address, borrowAmount, owner.address);

        console.log(
          "After Flashloan: DAI Balance of owner",
          ethers.utils.formatUnits(await dai.balanceOf(owner.address), 18)
        );
