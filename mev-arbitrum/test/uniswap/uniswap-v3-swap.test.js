...
describe("UniswapV3 SWAP", () => {
  ...
  it(" WETH => DAI on Uniswap  => 0.03%", async () => {
    const { twap, owner, ethAmount, dai , testChainlink } = await loadFixture(deployFixture);
  ...
  it(" WETH => DAI on Uniswap  => 0.03%", async () => {
    ...
    amount = 10n ** 18n
    daiAmount = 1699n * 10n ** 18n; // 1 eth
    price = await testChainlink.getLatestPriceETH()
    await twap.swapExactOutputSingle(dai.address, daiAmount, amount)
    console.log(ethers.utils.formatEther(await dai.balanceOf(owner.address)))
    console.log(price)
